...
########################################################################################################################
## Disconnected OpenShift Assisted Installer Service, Low-side Bootstrap Script
##
...
## Installer Service.
##
## This script assumes that this host is connected to the internet.
##
## Prerequisites:
##
## - The host must be running RHEL 8.4+ and already subscribed to RHSM/Satellite, etc - see mirror-vm.preflight.sh
## - The host must have a static IP address assigned to it
##
########################################################################################################################
...
mkdir -p ${MIRROR_DIR}/cni/net.d/

cat > ${MIRROR_DIR}/cni/net.d/$MIRROR_VM_ISOLATED_BRIDGE_IFACE.conflist <<EOF
{
  "cniVersion": "0.4.0",
...
CUR_SUM_DNS_SVC=""
if [ -f "${MIRROR_DIR}/dns/dns-go-zones.service" ]; then
  CUR_SUM_DNS_SVC=$(md5sum ${MIRROR_DIR}/dns/dns-go-zones.service)
fi
cat > ${MIRROR_DIR}/dns/dns-go-zones.service <<EOF
[Unit]
...
EOF
NEW_SUM_DNS_SVC=$(md5sum ${MIRROR_DIR}/dns/dns-go-zones.service)
...
CUR_SUM_REG_SVC=""
if [ -f "${MIRROR_DIR}/mirror-registry.service" ]; then
  CUR_SUM_REG_SVC=$(md5sum ${MIRROR_DIR}/mirror-registry.service)
fi
...
NEW_SUM_REG_SVC=$(md5sum ${MIRROR_DIR}/mirror-registry.service)
...
###########################################
## Ingress Pod Service
cat > ${MIRROR_DIR}/mirror-ingress/mirror-ingress.service <<EOF
[Unit]
...
cat > ${MIRROR_DIR}/ai-svc/assisted-installer.service <<EOF
[Unit]
...
########################################################################################################################
## Set permissions for all the service scripts
